...
\item{They lack chlorophyll, are non-photosynthetic and have to get their own food by feeding on dead plants or animals. (Notice the lack of green is because of the lack of chlorophyll)}
...
Some mushrooms are poisonous. Hands should be washed thoroughly with soap and water after handling.
...
Mushrooms can be preserved through drying or preservation in 70\% alcohol in a sealed container. \\
\textbf{To dry the mushrooms:}
Boil the mushrooms in salt water for 5 minutes and then dry them in sunlight for a few days.
...
Monocotyledon seeds have only one cotyledon (seed leaf). Monocots have a fibrous root system, leaves with parallel venation, three part floral systems, and vascular bundles which are scattered. Examples of monocotyledons are maize and grasses.
...
\subsubsection{Preservation} 
Flowers and leaves can be dried in a book. Place the flower or leaf between two sheets of paper and then press these in the centre of a book. Add more weight on top of the book and wait a few weeks. These specimens will be very delicate but will last a long time.
...
\subsubsection{Killing}
An earthworm can be killed by placing it into a formaldehyde solution or methylated spirits, however for the case of dissection this might deform the body.\\
For dissection, the worm should be killed by indirect contact with ethanol*. The earthworm should be place in a sealed container containing a piece of cotton wool soaked with ethanol. It is important to make sure that there is no direct contact with the worm and the alcohol. To achieve this, the cotton wool can be tied using a string and hung above the earthworm in a sealed bottle. After 5-10 minutes the earthworm will be dead and ready for dissection.

\subsubsection{Dissection}
You can observe the internal structures of an earthworm.\\
\textbf{Materials:} Razor blade, pins (or thorns), cardboard box\\
\textbf{Procedure:}
\begin{enumerate}
\item Lay the earthworm on the cardboard box. Pin both the anterior (head) and posterior ends using the thorns.
\item Carefully cut the cuticle of the earthworm laterally from head to anus and pin back the skin.
\item Observe the interior structure of the worm.
\end{enumerate}

\subsection{Phylum Arthropoda}
Organisms in this phylum have jointed appendages and an exoskeleton made of chitin. There are 5 classes in this phylum: Insecta, Crustacea, Arachnida, Diplopoda, and Chilopoda.
...
\subsubsection{Preservation} 
Live insects can be kept in a clear bottle and fed grass clippings. Dead insects can be preserved for a few months by placing them in methylated spirits. Small dead insects can also be dried in the sun and then sealed in an airtight container. If the insects are dried, special care must be taken to avoid the attraction of ants which will destroy the specimen.
...
\subsubsection{Collection}
Fresh water crabs can be found in rivers in Mbeya, Iringa, and Songea. Otherwise, shrimp can be purchased in some bus stands or markets.
